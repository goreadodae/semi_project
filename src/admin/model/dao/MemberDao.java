...
/*
	public ArrayList<Member> viewAllMember(Connection conn) {
		...
		try {
			...
			while(rset.next())
			{
				...
				m.setNickName(rset.getString("niceName"));
				list.add(m);
			}
	...
	public ArrayList<Member> getCurrentPage(Connection conn, int currentPage, int recordCountPerPage) {
		...
		String query ="select * from " + 
				...
				+ "over(order by member_no desc)as num from member) " + 
	...
	public String getPageNavi(Connection conn, int currentPage, int recordCountPerPage, int naviCountPerPage) {
		...
		// 토탈 게시물이 124개 라고 했을때 124%10을 한 후 +1 만큼
		// 만들어야 함
		// 만약 나머지가 0으로 덜어진다면 +1을 하지 않음
		...
		// 사용자가 현제 페이지가 1페이지인데 더 아래 페이지를 요청할 경우
		...
		// 토탈 navi 가 122라고 할때!(현재 페이지는 121이라고 할때)
		// 시작 navi-> ((121-1)/5)*5+1 = 121
		// 끝 navi => 121 + 5 - 1 = 125
		// 이렇게 때문에 마지막 navi 부분은 아래 코드를 추가 해야 함
		...
		if(needPrev)	//시작이 1페이지가 아니라면!
		{
			...
			sb.append("<a class='page-link' href='/memberMgt?currentPage=" + (startNavi - 1) + " aria-label='previous'>");
		...
		for(int i=startNavi; i<=endNavi; i++)
		{
			if(i==currentPage)
			{
				...
				sb.append(" href='/memberMgt?currentPage=" + i + "'>" + i + "</a>");
			}
			else
			{
		...
		{
			sb.append("<li class=\'page-item\'>");
			sb.append("<a class='page-link' href='/memberMgt?currentPage=" + (endNavi + 1) + " aria-label='Next'>");
			sb.append("<span aria-hidden='true'>&laquo;</span>");
